...
def setup_logging(
    ...
    if not log_dir:
        log_dir = os.getcwd()
        logging.warning(f"Logger directory not defined. Default values will be used: log_dir = {log_dir} ", Warning)

    path_env = os.getenv(env_key, None)
    if path_env is not None and log_config is not None:
        logging.warning("Two logger configuration provided, from environment variables and from path file. "
    ...
    if log_config:
        ...
        if rank is not None or rank == 0:
            log_config = Path(__file__).absolute().parent / 'logger_config.json'
        else:
            log_config = Path(__file__).absolute().parent / 'logger_config_no_console_info.json'
