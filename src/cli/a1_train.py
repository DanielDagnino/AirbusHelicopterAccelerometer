...
from dataset.ds import DatasetAirbusHelicopterAccelerometer
from losses_metrics import get_loss, get_metric
from models.model_zoo import Feature2DAirbusHelicopterAccelerometer
from models.helpers import save_checkpoint, load_checkpoint
from optimizer import get_optimizer
from optimizer.clip import Clipper
from scheduler import ReduceLROnPlateau
from scheduler import get_scheduler, set_optimizer_lr
from trainer.trnr import trainer
from utils.general.computer import get_max_number_workers, choose_device
from utils.general.custom_yaml import init_custom_yaml
from utils.general.modifier import dict_modifier
from utils.logger.logger import setup_logging
from utils.torch.dataparallel import naked_model
...
def main_worker(this_gpu, n_gpus_per_node, cfg):
    ...
    if cfg.engine.model.name == Feature2DAirbusHelicopterAccelerometer.__name__:
        logger.info(f"{Feature2DAirbusHelicopterAccelerometer.__name__} building")
    ...
    metric_functs = dict()
    for stage in stages_trainer_sorted:
        for metric_definition in cfg.metric.get(stage):
            args = metric_definition.args
            metric_instance = get_metric(metric_definition.name, args)
            metric_functs.setdefault(stage, dict()).update({metric_instance.alias: metric_instance})
    ...
    for stage in stages_trainer_sorted:
        ...
        if cfg.dataset.name == DatasetAirbusHelicopterAccelerometer.__name__:
            dataset = DatasetAirbusHelicopterAccelerometer(stage=stage, **cfg.dataset.get(stage), rank=cfg.rank)
        else:
    ...
    for epoch in range(epoch_start, cfg.train.max_epochs):
        ...
        # Save.
        if metric_best is None or metric["valid"] > metric_best:
            loss_best = loss["valid"]
            metric_best = metric["valid"]
            metric_patient = 0
            save_checkpoint(save_model_dir / 'best.ckpt', model=model, optimizer=optimizer, scaler=scaler,
                            lr_scheduler=lr_scheduler, epoch=epoch, loss=loss_best, metric=metric_best,
                            patient=metric_patient)
            fn = f'E{str(epoch).zfill(3)}_L{loss["valid"]:.4f}_M:{metric["valid"]:.4f}_L{loss[name_save_ckpt]:.4f}_M:{metric[name_save_ckpt]:.4f}.ckpt'
...
def run(cfg_fn, encoder_name, transf_degree):
    # Configuration.
    init_custom_yaml()
    ...
    cfg = dict_modifier(
        ...
        pre_modifiers={
            ...
            "TIME": f"{datetime.datetime.now():%Y_%m_%d_%H_%M_%S}",
